...
def main(out=None):
    ...
    # Top-level layout: TOP zebra section (panels a-e), BOTTOM monkey section
    # (panels f-h). Source layout:
    ...
    # so each is rendered square, matching the 250x250 PNG aspect).
    a_sub = left[0].subgridspec(2, 1, height_ratios=[0.22, 1.0], hspace=0.05)
    ax_a_dots = fig.add_subplot(a_sub[0])
    a_thumbs_row = a_sub[1].subgridspec(1, 3, wspace=0.06)
    ax_a_thumb_list = [fig.add_subplot(a_thumbs_row[0, i]) for i in range(3)]
    panel_a(ax_a_dots, ax_a_thumb_list, zebra)

    ax_c = fig.add_subplot(left[1])
    panel_c(ax_c, zebra)

    ax_d = fig.add_subplot(left[2])
    panel_d(ax_d, zebra)
    ...
    axes_b = {
        ...
        "l1": fig.add_subplot(b_sub[1]),
        "p1": fig.add_subplot(b_sub[2]),
        "f1": fig.add_subplot(b_sub[3]),
        "t2": fig.add_subplot(b_sub[4]),
        "l2": fig.add_subplot(b_sub[5]),
        "p2": fig.add_subplot(b_sub[6]),
        "f2": fig.add_subplot(b_sub[7]),
    }
    panel_b(axes_b, zebra)

    ax_e = fig.add_subplot(right[1])
    panel_e(ax_e)

    # ============================================================
    # BOTTOM SECTION — monkey (panels f-h)
    ...
    panel_h(ax_h, monkeys)

    if out is None:
        out = os.path.join(HERE, "figure4_remade.png")
